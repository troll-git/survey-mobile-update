...
const helpGroupOrder = ['auth', 'core', 'client', 'info', 'publish', 'build', 'credentials', 'notifications', 'url', 'webhooks', 'upload', 'eject', 'experimental', 'internal'];
    ...
    new (_xdl().PackagerLogsStream)({
      ...
      onFinishBuildBundle: (err, startTime, endTime) => {
        ...
        if (bar) {
          ...
          if (err) {
            _log().default.log(_chalk().default.red('Failed building JavaScript bundle.'));
          } else {
            const totalBuildTimeMs = endTime.getTime() - startTime.getTime();

            _log().default.log(_chalk().default.green(`Finished building JavaScript bundle in ${totalBuildTimeMs}ms.`));
...
async function bootstrapAnalyticsAsync() {
  _xdl().Analytics.initializeClient('vGu92cdmVaggGA26s3lBX6Y5fILm8SQ7', packageJSON.version);

  _xdl().UnifiedAnalytics.initializeClient('u4e9dmCiNpwIZTXuyZPOJE7KjCMowdx5', packageJSON.version);
        ...
        if (chunk.code) {
          switch (chunk.code) {
            ...
            case _xdl().NotificationCode.TICK_PROGRESS_BAR:
              {
                const spinner = _log().default.getProgress();
