...
async function resolveOptionsAsync(projectRoot, options) {
  ...
  if (options.scheme === true) {
    ...
    if (schemes.length > 1) {
      options.scheme = await (0, _prompts().selectAsync)({
        message: 'Select a scheme',
        choices: schemes.map(value => {
          const isApp = value.type === _configPlugins().IOSConfig.Target.TargetType.APPLICATION;

          return {
            value: value.name,
            title: isApp ? _chalk().default.bold(value.name) + _chalk().default.gray(' (default)') : value.name
          };
        })
      }, {
      ...
      _log().default.log(`Auto selecting only available scheme: ${options.scheme}`);
    }
  }
