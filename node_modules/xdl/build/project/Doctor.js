...
async function validateWithSchema(projectRoot, {
  ...
  try {
    ...
    if (e instanceof _schemer().SchemerError) {
      schemaErrorMessage = `Error: Problem${e.errors.length > 1 ? 's' : ''} validating fields in ${configName}. ${(0, _TerminalLink().learnMore)('https://docs.expo.io/workflow/configuration/')}`;
  ...
  if (validateAssets) {
    try {
      ...
      if (e instanceof _schemer().SchemerError) {
        assetsErrorMessage = `Error: Problem${e.errors.length > 1 ? '' : 's'} validating asset fields in ${configName}. ${(0, _TerminalLink().learnMore)('https://docs.expo.io/')}`;
...
async function _validateReactNativeVersionAsync(exp, pkg, projectRoot, sdkVersions, sdkVersion) {
  ...
  if (!/expo\/react-native/.test(reactNative)) {
    _internal2().ProjectUtils.logWarning(projectRoot, 'expo', `Warning: Not using the Expo fork of react-native. ${(0, _TerminalLink().learnMore)('https://docs.expo.io/')}`, 'doctor-not-using-expo-fork');
