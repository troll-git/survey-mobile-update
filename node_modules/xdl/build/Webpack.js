...
async function startAsync(projectRoot, options = {}, deprecatedVerbose) {
  if (typeof deprecatedVerbose !== 'undefined') {
    throw new (_internal().XDLError)('WEBPACK_DEPRECATED', 'startAsync(root, options, verbose): The `verbose` option is deprecated.');
  }
...
async function bundleAsync(projectRoot, options) {
  ...
  if (typeof env.offline === 'undefined') {
    try {
      ...
      if (expoConfig.exp.sdkVersion) if (_internal().Versions.lteSdkVersion(expoConfig.exp, '38.0.0')) {
        env.offline = true;
      }
    } catch {// Ignore the error thrown by projects without an Expo config.
    }
  }
  ...
  if (!hasSWPlugin) {
    _internal().ProjectUtils.logInfo(projectRoot, WEBPACK_LOG_TAG, _chalk().default.green(`Offline (PWA) support is not enabled in this build. ${_chalk().default.dim((0, _internal().learnMore)('https://expo.fyi/enabling-web-service-workers'))}\n`));
  }
}
