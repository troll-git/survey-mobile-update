...
async function openAppAsync(device, {
  ...
  const targetActivityURI = mainActivity.includes('.') ? mainActivity : [packageName, mainActivity].filter(Boolean).join('/.');
  const openProject = await getAdbOutputAsync(adbPidArgs(device.pid, 'shell', 'am', 'start', // FLAG_ACTIVITY_SINGLE_TOP -- If set, the activity will not be launched if it is already running at the top of the history stack.
