...
async function ensureSimulatorOpenAsync({
  ...
  if (!(await _internal().SimControl.isSimulatorAppRunningAsync())) {
    ...
    await _internal().SimControl.openSimulatorAppAsync({
      udid
    });
  ...
  if (!udid) {
    ...
    if (simulatorOpenedByApp !== null && simulatorOpenedByApp !== void 0 && simulatorOpenedByApp.udid) {
      udid = simulatorOpenedByApp.udid;
    } else {
      var _getDefaultSimulatorD;

      udid = (_getDefaultSimulatorD = _getDefaultSimulatorDeviceUDID()) !== null && _getDefaultSimulatorD !== void 0 ? _getDefaultSimulatorD : (await getFirstAvailableDeviceAsync()).udid;
    }
  }
...
async function waitForActionAsync({
  ...
  do {
    await (0, _internal().delayAsync)(interval);
...
async function waitForSimulatorAppToStart() {
  return waitForActionAsync({
    interval: 50,
    action: _internal().SimControl.isSimulatorAppRunningAsync
  });
}
...
async function waitForDeviceToBootAsync({
  ...
  return waitForActionAsync({
    action: () => {
      return _internal().SimControl.bootAsync({
        udid
      });
    }
  });
...
async function isExpoClientInstalledOnSimulatorAsync({
  ...
  return !!(await _internal().SimControl.getContainerPathAsync(udid, 'host.exp.Exponent'));
}
...
async function expoVersionOnSimulatorAsync({
  ...
  const localPath = await _internal().SimControl.getContainerPathAsync(udid, 'host.exp.Exponent');
...
async function installExpoOnSimulatorAsync({
  ...
  const setWarningTimer = () => {
    ...
    return setTimeout(() => {
      ...
      _internal().Logger.global.info('This download is taking longer than expected. You can also try downloading the clients from the website at https://expo.io/tools');
...
async function upgradeExpoAsync(options = {}) {
  ...
  if (_lastUrl) {
    _internal().Logger.global.info(`\u203A Opening ${_chalk().default.underline(_lastUrl)} in Expo`);

    await _internal().SimControl.openURLAsync({
      udid: simulator.udid,
      url: _lastUrl
    });
    _lastUrl = null;
...
async function openUrlInSimulatorSafeAsync({
  ...
  try {
    ...
    await (0, _profileMethod().profileMethod)(_internal().SimControl.openURLAsync, 'SimControl.openURLAsync')({
      url,
      udid: simulator.udid
    });
  } catch (e) {
...
async function assertDevClientInstalledAsync(simulator, bundleIdentifier) {
  if (!(await _internal().SimControl.getContainerPathAsync(simulator.udid, bundleIdentifier))) {
    throw new Error(`The development client (${bundleIdentifier}) for this project is not installed. ` + `Please build and install the client on the simulator first.\n${(0, _internal().learnMore)('https://docs.expo.io/clients/distribution-for-ios/#building-for-ios')}`);
  }
...
async function sortDefaultDeviceToBeginningAsync(devices) {
  var _getDefaultSimulatorD2;

  const defaultUdid = (_getDefaultSimulatorD2 = _getDefaultSimulatorDeviceUDID()) !== null && _getDefaultSimulatorD2 !== void 0 ? _getDefaultSimulatorD2 : (await getFirstAvailableDeviceAsync()).udid;
