...
async function startAsync(projectRoot, {
  ...
  if (options.webOnly) {
    await _internal().Webpack.restartAsync(projectRoot, { ...options,
      port: options.webpackPort
    });

    _internal().DevSession.startSession(projectRoot, exp, 'web');
...
async function stopInternalAsync(projectRoot) {
  ...
  await Promise.all([_internal().Webpack.stopAsync(projectRoot), new Promise((resolve, reject) => {
    if (serverInstance) {
  ...
  }), (0, _internal().stopExpoServerAsync)(projectRoot), (0, _internal().stopReactNativeServerAsync)(projectRoot), async () => {
